// ---------------------------------------------------------------------------
// File: TNMObject.cc
// Description: Prototype of a generic ADL object class.
...
TNMObject::TNMObject(string name, 
...
TNMObject::TNMObject(const TNMObject& p)
  : TLorentzVector(),
    UID(p.UID),
    PID(p.PID),
    ID(p.ID),
...
bool TNMObject::operator<(const TNMObject& p) const
{
  return p.Order < this->Order;
}

TNMObject& TNMObject::operator=(const TNMObject& rhs)
{
  if ( this != &rhs )
    {
      TNMObject p(rhs); // call copy constructor
      if ( p.Pt() > 0 )
	SetPtEtaPhiM(p.Pt(), p.Eta(), p.Phi(), p.M());
      UID    = p.UID;
      PID    = p.PID;
...
TNMObject& TNMObject::operator=(const TLorentzVector& rhs)
{
  if ( this != &rhs )
    {
      SetPtEtaPhiM(rhs.Pt(), rhs.Eta(), rhs.Phi(), rhs.M());
      UID    = ++s_UID;
      PID    = 0;
      ID     =-1;
      Status = 0;
...
string TNMObject::name(int pdgid)
{
  if ( firstName )
    {
      firstName = false;
